...
The current application version is `0.8.1`. Fore the Record uses a shared application version and keeps user-facing release notes separate from developer-only changes:
...
Apply database migrations before deploying the application. The v0.8.0 migrations classify existing rounds as casual individual rounds, add competition metadata, and permit null score fields only for validated team record entries. Do not run the full RapidAPI import on the current 200-request plan. Course and scorecard provider calls use the quota-safe, on-demand process documented in the [course catalogue guide](docs/COURSE-CATALOGUE.md).
