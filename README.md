...
Round Comparison lets a player place two verified individual rounds from the same course and hole layout side by side, including rounds from different tees. It compares gross score, score to par, Stableford where both cards use it, score differential, reconstructed Handicap Index outcome, Front 9 and Back 9 totals, optional statistics, and every comparable hole. Pickups and unrecorded details stay visibly unavailable, and either complete card can be opened directly in private Round History.
...
The private Notification Centre brings friend and challenge activity, shared rounds, support updates, scorecard-review decisions, group activity, and achievements into one newest-first inbox. Its navigation badge and read state follow the signed-in player across devices, and each entry opens the relevant part of the record without exposing another player's private information.

Players can personalise their record with an optional profile picture, short bio, and location. A separate visibility choice controls whether accepted friends can see those details across friend profiles and private groups without changing Handicap Index privacy. Profile pictures remain in private storage and are opened only through short-lived links; replacing, removing, or deleting the account also cleans up the stored image.

The private Statistics Dashboard lets each player choose and reorder scoring, Handicap Index, putting, driving, approach, short-game, penalty, and bunker cards. Saved date, course, tee, round-type, and round-length filters follow the account across devices, and every result includes its actual sample without inventing values for missing optional statistics.

Accepted friends can run private named knockout competitions for two to 16 players. Invitees answer before the organiser starts the automatic Match Play draw, byes advance safely, decisive margins move winners into their next fixtures, and the completed bracket records its champion without changing anyone’s round history or Handicap Index.

The Profile screen includes a private golf bag with up to 14 active clubs in the player's preferred order. Players can record the club type plus optional brand, model, nickname, loft, shaft flex, and carry distance, then edit, reorder, archive, restore, or permanently delete equipment. Golf bag details stay private and never alter rounds, statistics, or Handicap Index calculations.

Accepted friends can join private multi-event tournament series. Organisers schedule dated events, record finishing positions with flexible points, and close or cancel the series, while automatic standings track events played, wins, podiums, and total points. Tournament series remain social records and never alter scorecards or Handicap Index calculations.
...
The current application version is `0.53.0`. Fore the Record uses a shared application version and keeps user-facing release notes separate from developer-only changes:
