...
The application features are still under development. The core handicap calculation domain logic, backend profile creation and retrieval, transactional round logging with scorecard capping and automatic handicap recalculation, the round-history API, and the frontend authentication, catalogue search, total-score round-entry, round-history, and support-request flows are implemented. Supabase Auth provides email-and-password registration, existing-profile claiming through verified email ownership, login, logout, persistent sessions, and password recovery. Authenticated API requests derive profile ownership from the verified access token rather than trusting a browser-stored profile ID. The course catalogue importer can safely normalize and upsert the confirmed RapidAPI club, course, and tee-set structures. Because the current provider allowance cannot support a full import, **Search catalogue** uses the local database first and automatically checks provider club names only after a miss, with the possible request cost shown to the player. Successful matches can be saved once for reuse across home-club, Courses, and Round Entry searches. Players can search imported clubs when choosing a home club, search clubs and courses independently in Courses, and search a bounded set of rated tees while entering a round. Missing clubs or courses can be sent directly to the administrator. The history view lists rounds newest first, displays score and course details, and identifies the rounds used in the current Handicap Index. Signed-in players can submit ideas, site issues, data corrections, and missing-course details, view only their own request history, and reply within the original conversation. The sole administrator has a protected portal with operational totals, recent registrations, paginated player search, and a searchable support-request queue. The administrator can request more information, reply, and update submission statuses through audited controls; user and round management remains read-only. The production application is deployed on Vercel. Total-score entry uses gross score as adjusted gross score because Net Double Bogey capping requires a complete hole-by-hole scorecard.

## Roadmap and releases

The current application version is `0.6.0`. Fore the Record uses a shared application version and keeps user-facing release notes separate from developer-only changes:
...
Apply database migrations before deploying the application. The v0.6.0 catalogue migration adds provider identifiers and course metadata without removing existing club, course, tee, home-club, or round relationships. Do not run the full RapidAPI import on the current 200-request plan. The application uses the quota-safe, on-demand process documented in the [course catalogue guide](docs/COURSE-CATALOGUE.md).
