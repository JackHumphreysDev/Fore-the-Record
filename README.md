...
The application features are still under development. The core handicap calculation domain logic, backend profile creation and retrieval, transactional round logging with scorecard capping and automatic handicap recalculation, the round-history API, and the frontend authentication, catalogue search, hole-by-hole round-entry, round-history, support-request, and player-update flows are implemented. Supabase Auth provides email-and-password registration, existing-profile claiming through verified email ownership, login, logout, persistent sessions, and password recovery. Authenticated API requests derive profile ownership from the verified access token rather than trusting a browser-stored profile ID. The course catalogue importer can safely normalize and upsert the confirmed RapidAPI club, course, and tee-set structures. Because the current provider allowance cannot support a full import, **Search catalogue** uses the local database first and automatically checks provider club names only after a miss, with the possible request cost shown to the player. Successful matches can be saved once for reuse across home-club, Courses, and Round Entry searches. Round Entry retrieves and saves the selected tee's provider scorecard on demand. Players can classify entries as casual rounds, individual competitions, or team competitions. Scored individual rounds require the signed total and all 18 holes and may affect the Handicap Index; scoreless team entries are stored only in playing history. A score mismatch is blocked with the exact difference. If hole definitions are unavailable, the player can enter par and stroke index, leave yardage blank, and save the round for administrator review. Pending cards remain provisional and do not affect the Handicap Index until approved. The administrator can amend and approve the scorecard definition without changing player strokes, after which the round and handicap are recalculated. The history view shows round type, competition details, scorecard review status, and counting rounds. Signed-in players can submit ideas, site issues, data corrections, and missing-course details, view only their own request history, and reply within the original conversation. The player-facing **What's New** tab presents curated feature summaries and release dates without exposing version numbers or internal development notes. The sole administrator has a protected portal with operational totals, recent registrations, paginated player search, secure player invitations, profile editing, suspension/restoration, guarded account deletion, audited round correction and deletion, a searchable support-request queue, and a scorecard-review queue. Every changed or removed scored round triggers a complete Handicap Index recalculation. The sole administrator account cannot be edited, suspended, or deleted through the portal. The production application is deployed on Vercel.

## Roadmap and releases

The current application version is `0.19.0`. Fore the Record uses a shared application version and keeps user-facing release notes separate from developer-only changes:
