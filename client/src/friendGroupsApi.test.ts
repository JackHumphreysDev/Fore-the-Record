...
const player = { id: 'one', name: 'One', homeClub: null, hasProfileImage: false }
const group = { id: 'group', name: 'Sunday golfers', description: 'Weekend league', image: null, ownerId: 'one', isOwner: true, createdAt: 'date', updatedAt: 'date', players: [{ ...player, joinedAt: 'date', isOwner: true }] }
...
describe('friend group API contracts', () => {
  it('accepts complete group and leaderboard responses', () => {
    ...
    expect(isFriendGroupLeaderboardResponse({ group, period: '90_DAYS', startsOn: '2026-06-25', recentRounds: [], standings: [{ player, roundsPlayed: 2, stablefordPoints: 70, averageGross: 82.5, grossRounds: 2, bestGross: 80, handicapImprovement: 1.2 }] })).toBe(true)
    expect(isFriendGroupMessagesResponse({ messages: [{ id: 'message', groupId: 'group', authorId: 'one', body: 'Great round', createdAt: 'date', canDelete: true, author: player, round: null }] })).toBe(true)
  })
  ...
  it('ranks higher totals first and lower gross scores first', () => {
    ...
    const standings = [
      { ...base, player: { id: 'a', name: 'A', homeClub: null, hasProfileImage: false }, roundsPlayed: 1 },
      { ...base, player: { id: 'b', name: 'B', homeClub: null, hasProfileImage: false }, roundsPlayed: 3, bestGross: 80 },
    ]
