...
`GET /api/admin/scorecard-reviews` returns unresolved player-entered scorecard definitions with the associated player, tee, round, and locked hole strokes. `PATCH /api/admin/scorecard-reviews/:reviewId` accepts an approval with the complete selected 9 or all 18 corrected hole definitions, or a rejection. Approval stores the reusable tee holes and recalculates the applicable round values. Approved nine-hole rounds remain outside the Handicap Index until official expected-differential support is available. The player's submitted strokes are never editable or replaced.
